...
from configuration, including API model mapping and dynamic agent instantiation.
"""
...
class TestAgentModelEnum:
    ...
    def test_agent_model_enum_count(self):
        """Test that AgentModel enum has exactly the expected number of models."""
        expected_count = 5
...
class TestConfigurationBasedAgentCreation:
    ...
    def test_multiple_agent_creation_independence(self, mock_openai, mock_get_config):
        ...
        assert type(agents[0]) != type(agents[1]) if len(agents) > 1 else True
