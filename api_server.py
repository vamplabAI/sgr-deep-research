...
class ChatCompletionResponse(BaseModel):
    ...
    choices: List[Dict[str, Any]] = Field(..., description="The list of completion choices")
...
async def execute_sgr_research_streaming(messages: List[Dict[str, Any]]):
    ...
    conversation_messages = [
        {
            "role": "system",
            "content": PROMPTS["outer_system"]["template"].format(user_request=user_request),
        }
    ]
    ...
    while rounds < max_rounds:
        ...
        try:
            ...
            reasoning = await exec_reasoning_phase_api(conversation_messages, user_request, context)
            ...
            if reasoning.current_situation == "OpenAI API unavailable - using fallback" and rounds >= 2:
            ...
            action_results = await exec_action_phase_api(conversation_messages, reasoning, context)
...
async def exec_reasoning_phase_api(messages: List[Dict[str, Any]], task: str, context: Dict[str, Any]) -> ReasoningStep:
...
def exec_structured_output_reasoning_api(messages: List[Dict[str, Any]], task: str, context: Dict[str, Any]) -> Dict[str, Any]:
...
async def exec_action_phase_api(messages: List[Dict[str, Any]], reasoning: ReasoningStep, context: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def execute_tool_call_api(tool_call: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
...
def create_reasoning_chunk(reasoning: ReasoningStep, round_num: int, max_rounds: int) -> str:
...
def create_completion_chunk(reasoning: ReasoningStep, round_num: int, max_rounds: int) -> str:
...
def create_error_chunk(error_result: Dict[str, Any], round_num: int, max_rounds: int) -> str:
...
def create_final_response(messages: List[Dict[str, Any]], context: Dict[str, Any], rounds: int) -> ChatCompletionResponse:
            ...
            if content:
                try:
                    ...
                    if "answer" in data:
                        ...
                        final_content += f"Search results: {len(data['results'])} items found\n"
                except:
                    pass
