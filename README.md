...
Агент использует двухфазный подход (reasoning + action) и предоставляет **6 оптимизированных инструментов** для работы с файлами:
...
3. **ReadFileTool** - чтение содержимого файлов (с опциональным диапазоном строк)
4. **ListDirectoryTool** - просмотр содержимого директорий (с рекурсией)

#### 🔍 Инструменты поиска

5. **FindFilesFastTool** ⚡ - **Универсальный поиск файлов** - использует нативную команду `find`
   - Поддерживает паттерны имен (`*.py`, `config.*`)
   - Фильтры по размеру и дате модификации
   - 10-100x быстрее Python-аналогов
   - Автоматически исключает кеши, node_modules, .git
   
6. **SearchInFilesTool** - поиск текста/кода внутри файлов (grep-подобный)
...
└── Phase 2: Action
    ...
    └── File System Tools (6 tools)
        ...
        │   └── ListDirectoryTool
        └── Search
            ├── FindFilesFastTool ⚡ (native find - universal)
            └── SearchInFilesTool (grep-like)
```
