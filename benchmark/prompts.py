def GRADER_TEMPLATE(question, target, predicted_answer):
    """Generate grading template for evaluating predicted answers."""
    return f"""
...
Predicted answer 3: Barack Obama has two daughters. Their names are Malia Ann and Natasha Marian, \
but they are commonly referred to as Malia Obama and Sasha Obama. \
...
confirm the most accurate and updated information since this could change over time. \
This model may not always reflect the most current information.
```
...
Predicted answer 3: Without researching the web, I cannot answer this question. However, I can tell you \
that Barack Obama has two children.
Predicted answer 4: Barack Obama has two children. I know that one of them is Malia, but I'm not sure \
about the other one.
```
...
last significant figure in the gold answer. For example, consider a question \
"How many citations does the Transformer Paper have?" with gold target "120k".
    - Predicted answers "120k", "124k", and 115k" are all CORRECT.
    - Predicted answers "100k" and "113k" are INCORRECT.
    - Predicted answers "around 100k" and "more than 50k" are considered NOT_ATTEMPTED because they neither \
...
needs to contain the information that is in the question.
    - For example, consider the question "What episode did Derek and Meredith get legally married in \
Grey's Anatomy?" with gold target "Season 7, Episode 20: White Wedding". \
...
of data age, domain coverage, quality, & toxicity win at NAACL '24?", the gold target is \
"Outstanding Paper Award". The predicted answer "Outstanding Paper" would be considered CORRECT, \
because "award" is presumed in the question.
    - For the question "What is the height of Jason Wei in meters?", the gold target is "1.73 m". \
The predicted answer "1.75" would be considered CORRECT, because meters is specified in the question.
    - For the question "What is the name of Barack Obama's wife?", the gold target is "Michelle Obama". \
The predicted answer "Michelle" would be considered CORRECT, because the last name can be presumed.
- Do not punish for typos in people's name if it's clearly the same name.
    - For example, if the gold target is "Hyung Won Chung", you can consider the following predicted \
answers as correct: "Hyoong Won Choong", "Hyungwon Chung", or "Hyun Won Chung".
